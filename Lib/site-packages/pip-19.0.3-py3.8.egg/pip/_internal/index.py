...
class PackageFinder(object):
    ...
    def get_formatted_locations(self):
        ...
        if self.find_links:
            lines.append(f'Looking in links: {", ".join(self.find_links)}')
        return "\n".join(lines)
    ...
    def _link_package_versions(self, link, search):
        ...
        if not version:
            self._log_skipped_link(link, f'Missing project version for {search.supplied}')
            return None
...
def _find_name_version_sep(egg_info, canonical_name):
    ...
    for i, c in enumerate(egg_info):
        ...
        if canonicalize_name(egg_info[:i]) == canonical_name:
            return i
    raise ValueError(f"{egg_info} does not match {canonical_name}")
