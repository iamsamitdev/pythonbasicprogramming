...
def unpack_file(
    ...
    if (content_type == 'application/zip' or
        ...
        Subversion(f'svn+{link.url}').unpack(location)
    else:
        # FIXME: handle?
...
def call_subprocess(
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == 'raise':
            ...
            raise InstallationError(
                f'Command "{command_desc}" failed with error code {proc.returncode} in {cwd}'
            )
    ...
    return ''.join(all_output) if not show_stdout else None
